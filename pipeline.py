...
def _process_tile(
    *,
    ...
    try:
        ...
        if not img_bytes:
            return {"ok": False, "detections": [], "raw_path": "", "boxed_path": "", "det_sem_latlon": 0}
        ...
        # --- CORREÇÃO ROBUSTEZ (Dimensões Dinâmicas) ---
        # Calcula dimensões esperadas baseado na config, em vez de hardcoded 1280
        try:
            ts_w, ts_h = map(int, tile_size.split('x'))
            expected_w = ts_w * tile_scale
            expected_h = ts_h * tile_scale
        except Exception:
            expected_w, expected_h = 1280, 1280 # Fallback apenas se parse falhar
        ...
        return {"ok": False, "detections": [], "error": str(e)}
...
def _pipeline_core(sub_id: str, lat: float, lon: float, raio_m: float) -> dict:
    ...
    # Nova Flag de Segurança Financeira
    ...
    # --- SMART SCAN (COM TRAVA DE SEGURANÇA) ---
    ...
    if mascara:
        ...
        # CORREÇÃO ROBUSTEZ: Verifica se deve abortar ou continuar
        if osm_required:
            error_msg = f"⛔ [SMART SCAN] ABORTANDO: OSM não retornou dados e OSM_REQUIRED=True. (Evitando custo excessivo)"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
        else:
            logger.warning("⚠️ [SMART SCAN] Nenhuma edificação encontrada ou falha no OSM. Usando grid completo (Custo $$$).")
    # -------------------------------------------
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        for i, (t_lat, t_lon) in enumerate(tiles, 1):
            futures.append(ex.submit(_process_tile, sub_id=sub_id, i=i, t_lat=t_lat, t_lon=t_lon, zoom=zoom, tile_size=tile_size, tile_scale=tile_scale, raw_dir=raw_dir, boxed_dir=boxed_dir))
    ...
    logger.info(
        "[5/6] Impacto ok | duck=%s | mmgd=%s | gen=%.1fkW | rede=%.1fkW (Area Based)",
